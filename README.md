...
Validate and Compile the Code = Maven compiles the code and checks for syntax or build errors.

Test the Code = Runs unit tests using the Maven Surefire plugin.

Quality Check and Security Analysis = Executes SonarQube analysis using sonar-maven-plugin. Sends the code metrics to the SonarQube server for review.

Generate Build Artifact = Packages the code into a .war file using Maven (mvn clean package).

Upload Artifact to Nexus = The generated WAR file is uploaded to the Nexus Repository Manager.
...
Deploy the Artifact = Deploys the WAR to Apache Tomcat server using Jenkins Deploy to Container Plugin.
